...
import { markdown as markdownData, open_in_new_tab as openInNewTab } from 'peepsodata';
...
const compileMarkdown = html => {
	...
	$markdown.each(function() {
		...
		try {
			...
			// Respect "open link in new tab" config.
			if (+openInNewTab) {
				html = html.replace(/<a([^>]+href=[^>]+)>/gi, function(match, attrs) {
